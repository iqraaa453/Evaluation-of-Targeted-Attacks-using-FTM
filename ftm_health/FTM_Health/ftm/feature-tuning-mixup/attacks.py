...
def ftm_attack(
    ...
    # ✅ FIX: Build TI Gaussian kernel AFTER the attack loop starts so we can read
    # the actual channel count from grad_x at runtime (supports 1ch and 3ch inputs).
    # We defer kernel construction to first use inside the loop.
    gaussian_kernel = None  # Will be built on first TI use
    ...
    for t in range(consumed_iteration, num_iter):
        ...
        # ✅ FIX: Build Gaussian kernel dynamically based on actual channel count of grad_x.
        # This supports both 1-channel (grayscale/medical) and 3-channel (RGB) inputs,
        # instead of the previous hardcoded groups=3 which crashed on 1ch tensors.
        if 'T' in attack_type:
            ...
            if gaussian_kernel is None or gaussian_kernel.shape[0] != num_channels:
                ...
                stacked_kernel = np.stack([kernel] * num_channels)       # [C, H, W]
                stacked_kernel = np.expand_dims(stacked_kernel, 1)       # [C, 1, H, W]
            ...
            grad_x = F.conv2d(
                ...
                bias=None,
                stride=1,
                padding=((ti_kernel_size - 1) // 2, (ti_kernel_size - 1) // 2),
                groups=num_channels,   # ✅ dynamic: 1 for grayscale, 3 for RGB
            )
    ...
    # ✅ Only empty CUDA cache if CUDA is available
