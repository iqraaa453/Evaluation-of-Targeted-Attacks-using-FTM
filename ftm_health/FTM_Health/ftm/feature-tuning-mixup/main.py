...
def main(args):
    ...
    small_sized_models = [
        ...
        'xrv_chexnet', 'xrv_densenet_nih', 'xrv_densenet_chex', 'xrv_densenet_pc'
        # ✅ xrv_resnet50 removed from here — handled separately below (needs 512x512)
    ]

    # ✅ Native resolution per model — used for WrapperModel resize
    native_resolution = {
        'xrv_resnet50': 512,   # trained on 512x512 — must NOT resize to 224
    }
    # All others default to img_size (224)
    ...
    def make_wrapper(model_name):
        ...
        # ✅ Use model's native resolution if specified, else default img_size
        nat_res = native_resolution.get(model_name, img_size)
        resize  = (model_name in small_sized_models or is_med)

        model_loaded = load_model(model_name)

        # ✅ For xrv_resnet50 (512x512): wrap with an upsampler before WrapperModel
        if nat_res != img_size and is_med:
            class ResizeWrapper(torch.nn.Module):
                def __init__(self, inner, target_res):
                    super().__init__()
                    self.inner = inner
                ...
                def forward(self, x):
                    ...
                    return self.inner(x)
            model_loaded = ResizeWrapper(model_loaded, nat_res)
            resize = False  # WrapperModel should NOT resize again

        return WrapperModel(model_loaded, mean, std, resize, channels=channels)
    ...
    for k in range(num_batches):
        ...
        img_batch = torch.zeros(batch_size_cur, num_channels, img_size, img_size).to(device)
        ...
        for i in range(batch_size_cur):
            ...
            img_batch[i] = trn(raw_img.convert('L') if is_source_medical else raw_img.convert('RGB'))
    ...
    if args.eval:
        ...
        for model_name in target_model_names:
            ...
            print(f"     ✅ {model_name}: {rate:.1f}%  ({sum(r['Success'] for r in model_rows)}/{len(model_rows)})")
...
def argument_parsing():
    ...
    parser.add_argument("--image_dir",     type=str,   default="./data/images/images-224")
